...
### Visitor
На даному етапі у системі вже присутня велика ієрархія класів і для розширення функціоналу необхідно змінювати кожен клас, що порушує Single Responsibility і Open/Closed принципи. Для того, щоб додати нові функції у пристрої такі як перевірка статусу, потужності і конфігурації без втручання в код класів використоаємо патерн Visitor. Він дозволить створити нам універсального відвідувача, який буде проходитись по об'єктам і виконувати певну дію.<br>
Учасники:<br>
...
### Facade
На даному етапі система розумного будинку має складну структуру з множиною класів та компонентів, які взаємодіють між собою. Для спрощення взаємодії з цією складною системою та надання єдиної точки входу для клієнтського коду необхідно використати патерн Фасад. Це дозволить приховати складну логіку та надати простий інтерфейс для виконання типових операцій.
<br>Учасники:<br>
SmartHomeSystem - основний клас-фасад, який надає простий інтерфейс для роботи з системою<br>
SmartHomeController - компонент, який керує всією логікою, але прихований за фасадом<br>
Методи фасаду - AddDevice, TurnOnAllLights, SetTemperature, EnableSecurityMode та інші, які приховують комплексну логіку<br>

### Proxy
Система розумного будинку має потребу в контрольованому доступі до пристроїв, особливо при віддаленому керуванні. Для забезпечення додаткового рівня безпеки, логування операцій та контролю доступу до пристроїв використовується патерн Проксі. Цей патерн дозволяє створити об'єкт-замісник, який контролює доступ до реального об'єкта.<br>
Учасники:
IDeviceControl - інтерфейс, який визначає загальні методи для всіх пристроїв<br>
RemoteDeviceProxy - проксі-клас, який контролює доступ до реальних пристроїв<br>
Device - реальний клас, доступ до якого контролюється проксі<br>

### Bridge
У системі розумного будинку існує потреба в різних способах керування пристроями (голосове, через мобільний додаток, веб-інтерфейс) і різних реалізаціях цього керування (локальне, віддалене). Патерн Міст дозволяє відокремити абстракцію (інтерфейс керування) від реалізації, що дає можливість змінювати їх незалежно одне від одного. <br>
Учасники:
SmartHomeControlBridge - абстрактний клас, який визначає інтерфейс керування<br>
VoiceControl, MobileAppControl - конкретні абстракції, які визначають різні способи керування<br>
IControlImplementation - інтерфейс для конкретних реалізацій<br>
LocalControlImplementation, RemoteControlImplementation - конкретні реалізації, які виконують фактичне керування пристроями
